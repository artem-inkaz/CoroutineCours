...
//
//scope
//В примере выше все корутины находятся в рамках одного общего scope, из которого мы вызывали корутину 1.
// И сохраняется связь от родителя к дочерней. Т.е. вызвав scope.cancel, мы каскадно отменим все корутины,
// в том числе и те, которые находятся внутри coroutineScope, т.е. корутины 1_1 и 1_2.
//
//handler
//Обработчики CoroutineExceptionHandler не работают внутри coroutineScope,
// потому что ScopeCoroutine принимает ошибку от корутины 1_2 и говорит, что сможет ее обработать.
//
//Но в случае с supervisorScope это поведение меняется, чуть позже обсудим, как именно.

// возврат значения
//coroutineScope может вернуть результат своей работы
//
//val result = coroutineScope {
//   // ...
//   "result value"
//}
//Т.е. внутри coroutineScope можно не только новые корутины запускать, но и выполнять обычный код,
// результат которого станет результатом вызова coroutineScope.

//suspend функции
//Т.к. coroutineScope - это suspend функция, то ее вызов приостановит поток текущей корутины. Если вам это не надо, то вы можете поместить этот вызов в launch:
//
//launch {
//   coroutineScope {
//       // ...
...
//
//
//или в async, если ожидаете получить результат:
//
//async {
//   coroutineScope {
//       // ...
...
//
//
//Не забывайте при этом оборачивать coroutineScope в try-catch, если хотите, чтобы ошибка не распространялась вверх.
