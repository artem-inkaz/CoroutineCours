...
//Оператор retryWhen работает примерно так же, как и retry. Но он не принимает на вход количество
    // попыток. Вместо этого он дает нам в блок кода номер текущей попытки и ошибку.
    // А нам уже надо решить - возвращать true или false.
...
//Если тип ошибки NetworkErrorException и количество попыток меньше 5, то возвращаем true,
    // чтобы перезапустить Flow.

//Также в блоке кода retryWhen нам доступен метод emit(). Т.е. мы можем отправить какие-то свои данные
    // получателю.
