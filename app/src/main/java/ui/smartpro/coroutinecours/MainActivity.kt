...
//Потоки
//В плане потоков методы send и receive ведут себя так же, как и обычные suspend функции.
// Они приостанавливают корутину, если им приходится ждать отправления или получения данных.
// Когда корутина возобновляется, она может продолжить свое выполнение в другом потоке.
// Но в любом случае это будет поток из диспетчера корутины.
