...
// supervisorScope
//Если coroutineScope принимает ошибки от своих дочерних корутин и просто не шлет их дальше
// в родительскую корутину, то supervisorScope даже не принимает ошибку от дочерних.
// Это приводит к отличиям в обработке ошибок.
//
//Корутина 1_2, которая пытается передать ошибку наверх в ScopeCoroutine,
// получает отрицательный ответ и пытается обработать ошибку сама.
// Для этого она использует предоставленный ей CoroutineExceptionHandler.
// Если же его нет, то будет крэш, который не поймать никаким try-catch.
//
//Поэтому имеет смысл использовать CoroutineExceptionHandler внутри supervisorScope.
// В этом случае ошибка попадет в этот обработчик и на этом все закончится.
// Функция supervisorScope не выбросит исключение и не отменит остальные корутины внутри себя
// (т.е. корутину 1_1 в примере выше).
