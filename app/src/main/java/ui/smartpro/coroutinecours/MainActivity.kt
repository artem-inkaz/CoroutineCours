...
// переключаемся на Main поток и отображаем данные на экране.
// После чего корутина продолжает выполняться в своем диспетчере.
//
//Либо корутина может выполняться в Main потоке, а с помощью withContext мы можем переключиться на
// IO поток, чтобы сделать запрос на сервер или в БД, и получить результат обратно в Main поток.
