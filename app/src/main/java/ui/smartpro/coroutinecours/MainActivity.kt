...
//produceIn
//Оператор produceIn тоже использует produce, чтобы отправить данный ему Flow работать в новую корутину с каналом. Но при этом он сам не будет получать данные из канала. Он вернет этот канал нам.
//
//Т.е. под капотом это выглядит примерно так:
...
//В produce запускается переданный ему Flow, и все его данные отправляются в канал. И никакая Flow обертка не создается. Вместо этого мы получаем канал.
...
.produceIn(scope)
//Таким образом Flow конвертируется в канал.

//Обратите внимание на три важных момента.
//
//В produceIn надо передавать scope, который будет использован для запуска produce.
//
//Получившийся канал (в отличие от обычного Flow) не является Cold. Т.е. он начнет работу
    // сразу же, как мы его создадим.
//
