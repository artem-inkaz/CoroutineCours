...
//callbackFlow
//В примерах с channelFlow мы в корутине посылали данные в канал, они уходили в Flow,
// код корутины завершался, канал закрывался.
//
//Но последовательность действий может быть и немного другой. В корутине channelFlow мы можем не
// сразу постить данные в канал, а повесить какой-то колбэк или листенер, который позже будет
// срабатывать и вызывать метод send. Проблема здесь в том, что повесив колбэк,
// код корутины channelFlow завершается, а канал закрывается. И когда позже сработает колбэк,
// то он будет пытаться отправить данные в закрытый канал. И это приведет к ошибке.
//
//callbackFlow заточен именно под такие сценарии. В корутине мы подписываемся на какой-то колбэк.
// И мы уже не определяем, когда именно придут данные. А значит, корутина не должна пока заканчиваться
// и закрывать канал. Она должна ждать, пока канал не будет закрыт.
// Для этого используется метод awaitClose. Корутина не завершится, пока канал не будет закрыт.
//
//А канал будет закрыт, когда получатель данных отпишется от Flow (который является результатом
// билдера callbackFlow). Это приведет к срабатыванию awaitClose (в котором мы отписываемся от колбэка)
// и корутина завершит свою работу.
//
//callbackFlow отличается от channelFlow тем, что он по завершении кода корутины проверяет,
// что канал закрыт, т.е. что мы не забыли вызвать awaitClose в корутине.
// Если мы забудем, то сразу же получим ошибку. А в channelFlow мы получим ошибку,
    // только когда колбэк решит что-нить отправить.
