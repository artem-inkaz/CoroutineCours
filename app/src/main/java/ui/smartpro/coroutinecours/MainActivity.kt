...
// Смотрим на контекст корутины.
//
//Для нее был создан новый Job. Потому что у каждой корутины должен быть свой собственный Job,
// который отвечает за состояние корутины и ее результат.
//
//В качестве диспетчера в контекст корутины был помещен диспетчер по умолчанию, потому что из
// контекста scope не пришел никакой диспетчер, а он необходим для выполнения кода корутины.
// Иначе корутине просто неоткуда будет взять поток для выполнения своего кода.
